...
public class SubscriptionPanel{
    ...
    public void initialize() {
        ...
        if (subscriptions == null || subscriptions.isEmpty()) {
            showAlert("No Subscriptions", "You have no subscriptions.");
        } else {
            for (Channel channel : subscriptions) {
                channelListView.getItems().add(channel.getChannelName());
            }
        }

        channelListView.setOnMouseClicked(this::handleChannelSelection);
    }

    private void handleChannelSelection(MouseEvent event) {
        String selectedName = channelListView.getSelectionModel().getSelectedItem();
        if (selectedName == null) return;

        for (Channel ch : subscriptions) {
            if (ch.getChannelName().equals(selectedName)) {
                System.out.println("Navigating to channel: " + ch.getChannelName());
                break;
            }
        }
    }
